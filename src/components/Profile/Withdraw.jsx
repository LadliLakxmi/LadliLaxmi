...
const Withdraw = ({ user, fetchUserData }) => {
  ...
  const nextUpgradeCost = INDIVIDUAL_MAX_WITHDRAWAL_PER_LEVEL_FRONTEND[nextLevelToUpgrade]?.nextUpgradeCost || 0;
  ...
  useEffect(() => {
    ...
    const fetchWithdrawalStatus = async () => {
      try {
        const res = await axios.get("https://ladlilaxmi.onrender.com/api/v1/withdraw/my-requests", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        const pendingRequest = res.data.requests.some(req => req.status === 'pending');
      ...
      <div className="bg-gradient-to-br from-green-800 to-emerald-900 text-white p-8 md:p-10 rounded-2xl shadow-2xl w-full max-w-md mx-auto relative overflow-hidden transform transition-all duration-500 ease-in-out hover:scale-[1.01]">
        ...
        <div className="bg-green-700/50 border border-green-600 rounded-lg p-4 mb-6 text-base text-green-100 shadow-md">
          <p className="flex justify-between items-center mb-2">
            <span className="font-semibold flex items-center"><Wallet size={20} className="mr-2" />Main Wallet Balance:</span>
            <span className="font-bold text-yellow-300">₹{(Number(user?.walletBalance) || 0).toFixed(2)}</span>
          </p>

          <p className="flex justify-between items-center mb-2">
            <span className="font-semibold flex items-center"><Wallet size={20} className="mr-2" />Upgrade Wallet Balance:</span>
            <span className="font-bold text-yellow-300">₹{(Number(user?.upgradewalletBalance) || 0).toFixed(2)}</span>
          </p>
          ...
          {nextLevelToUpgrade <= Object.keys(INDIVIDUAL_MAX_WITHDRAWAL_PER_LEVEL_FRONTEND).length && (
          ...
          {!bankDetails ? (
            ...
            <div className="bg-yellow-700/30 border border-yellow-600 rounded-lg p-5 text-sm text-yellow-100 shadow-md">
              ...
              {bankDetails.upiId && ( // Display UPI ID if present
                <p className="mb-2"><strong>UPI ID:</strong> {bankDetails.upiId}</p>
              )}
